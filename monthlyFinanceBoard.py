...
#Created by Andy Park during Fall 2016 co-op term
...
stuff=['Created Column Location','Modified Column Location','Approved Column Location']
...
#Calculates difference for Manager
	...
	for rowNum in range(2,sheet.max_row - counter):
		managerSheet.cell(row=rowNum,column=calculatedManager).value= '='+stuff[2]+':'+stuff[2]+'-'+stuff[0]+':'+stuff[0]
...
#Calculates difference for Finance
	...
	for rowNum in range(2,sheet.max_row):
		financeSheet.cell(row=rowNum,column=calculatedFinance).value= '='+stuff[1]+':'+stuff[1]+'-'+stuff[2]+':'+stuff[2]
...
#Find the last column & also find the rows that have empty values - for different calculations
	...
	for colNum in range(1,sheet.max_column+1):
		...
		if search == "Created":
			stuff[0] = str(sheet.cell(row=1,column=colNum).column)
			inNumbers[0] = column_index_from_string(stuff[0])
		elif search == "Modified":
			stuff[1] = str(sheet.cell(row=1,column=colNum).column)
			inNumbers[1] = column_index_from_string(stuff[1])
		elif search == "Approved By Date":
			stuff[2] = str(sheet.cell(row=1,column=colNum).column)
			inNumbers[2] = column_index_from_string(stuff[2])
...
#Asks the name of the form
	...
	while keepGoing == True:
		...
		if int(choice) == 1:
			...
			formName= "Vendor Payment"
			keepGoing=False
			return formName
		elif int(choice) == 3:
			formName = "Payment Requisition"
			keepGoing=False
			return formName
		elif int(choice) == 4:
			formName = "CS Payment"
